...
in
{
  ...
  # Launches River WM compositor via ReGreet in a cage session

  # Assertion: Ensure wayland module is loaded (provides River compositor)
  assertions = [
    {
      assertion = config.programs.river-classic.enable or false;
      message = ''
        display-manager requires River compositor to be enabled.
        Please ensure modules/nixos/desktop/wayland.nix is imported in your role configuration.
      '';
    }
  ];
